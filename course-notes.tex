...
where $\bH\succeq 0$\footnote{$\bH\succeq 0 \Leftrightarrow \bx^T\bH \bx \geq 0, \quad \forall \bx \in \mathbb{R}^m$} (necessary for $f$ to be convex) and $\bH$ is symmetric.\footnote{We can assume $\bH$ is symmetric without loss of generality because if $\bH$ is not symmetric, we can replace it with $\hat \bH = \frac{1}{2}\left(\bH + \bH^\top\right)$, which yields an equivalent function $\hat f$}
